...
template <typename T>
IdentityMatrix<T>::IdentityMatrix(const dim_t n)
    : n_(n)
{
}

template <typename T>
dim_t IdentityMatrix<T>::height() const
{
  return n_;
}

template <typename T>
dim_t IdentityMatrix<T>::width() const
{
  return n_;
}

template <typename T>
T IdentityMatrix<T>::operator()(const dim_t i, const dim_t j) const
{
  return i == j ? 1 : 0;
}

template <typename T>
Matrix<T>::Matrix(const dim_t height, const dim_t width)
...
template <typename T>
Matrix<T>::Matrix(dim_t height, dim_t width, std::vector<T> entries)
    ...
    : height_(expr.height()), width_(expr.width())
{
  ...
  for (dim_t i = 0; i < height_; ++i) {
    for (dim_t j = 0; j < width_; ++j) {
...
template <typename T>
T Matrix<T>::operator()(const dim_t i, const dim_t j) const
{
  return entries_[i * width_ + j];
}

template <typename T>
T& Matrix<T>::operator()(const dim_t i, const dim_t j)
{
  return entries_[i * width_ + j];
}

template <typename T>
MatrixSlice<T> Matrix<T>::operator()(const dim_t i, const dim_t j,
                                     const dim_t height,
                                     const dim_t width)
{
...
template <typename T>
IdentityMatrix<T> Matrix<T>::identity(const dim_t n)
{
  return IdentityMatrix<T>(n);
}

template <typename T>
Matrix<T>& Matrix<T>::row_add(const dim_t i1, const dim_t i2,
                              const T& lambda)
{
  for (dim_t j = 0; j < width_; ++j) {
...
template <typename T>
Matrix<T>& Matrix<T>::row_mul(const dim_t i, const T& lambda)
{
  for (dim_t j = 0; j < width_; ++j) {
...
template <typename T>
Matrix<T>& Matrix<T>::row_swap(const dim_t i1, const dim_t i2)
{
  using std::swap;
...
template <typename T>
Matrix<T>& Matrix<T>::col_add(const dim_t j1, const dim_t j2,
                              const T& lambda)
{
  for (dim_t i = 0; i < height_; ++i) {
...
template <typename T>
Matrix<T>& Matrix<T>::col_mul(const dim_t j, const T& lambda)
{
  for (dim_t i = 0; i < height_; ++i) {
    entries_[i * width_ + j] *= lambda;
  }
...
template <typename T>
Matrix<T>& Matrix<T>::col_swap(const dim_t j1, const dim_t j2)
{
  using std::swap;
...
template <typename T>
MatrixSlice<T>::MatrixSlice(Matrix<T>& mat, const dim_t i,
                            const dim_t j, const dim_t height,
                            const dim_t width)
...
template <typename T>
T MatrixSlice<T>::operator()(const dim_t i, const dim_t j) const
{
  return mat_(i_ + i, j_ + j);
}

template <typename T>
T& MatrixSlice<T>::operator()(const dim_t i, const dim_t j)
{
  return mat_(i_ + i, j_ + j);
}
...
Matrix<T> operator*(const Matrix<T>& g, const Matrix<T>& f)
{
  ...
  for (dim_t i = 0; i < g.height(); ++i) {
    for (dim_t j = 0; j < f.width(); ++j) {
      T acc;

      for (dim_t k = 0; k < g.width(); ++k) {
        acc += g(i, k) * f(k, j);
      }
