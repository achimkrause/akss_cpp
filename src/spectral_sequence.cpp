...
const AbelianGroup& GroupSequence::get_group(const std::size_t index) {
	...
	if (!done_ && index > current_) {
		throw std::logic_error(
				"GroupSequence::get_group: Index is not yet set");
	}
...
const MatrixQ& GroupSequence::get_matrix(const std::size_t index) {
	...
	if (!done_ && index > current_) {
		throw std::logic_error(
				"GroupSequence::get_matrix: Index is not yet set");
	}
	std::map<std::size_t, std::tuple<AbelianGroup, MatrixQ>>::iterator pos =
			entries_.upper_bound(index);
...
void SpectralSequence::set_diff(TrigradedIndex pqs, std::size_t r,
		MatrixQ matrix) {
	TrigradedIndex diff_offset(-static_cast<int>(r), static_cast<int>(r) - 1,
			1);

	std::map<TrigradedIndex, GroupSequence>::iterator kers = kernels_.find(pqs);
	std::map<TrigradedIndex, GroupSequence>::iterator cokers = cokernels_.find(
			pqs + diff_offset);
	if (kers == kernels_.end()) {
		throw std::logic_error(
				"SpectralSequence::set_diff: Kernel is not set.");
	}
	...
	GroupWithMorphisms new_kernel = compute_kernel(prime_, matrix, X, Y,
			MatrixQRefList(), ref(from_X));
	GroupWithMorphisms new_cokernel = compute_cokernel(prime_, matrix, Y,
			ref(to_Y), MatrixQRefList());
...
const AbelianGroup SpectralSequence::get_e_ab(TrigradedIndex pqs, std::size_t a,
	...
	if (kers == kernels_.end()) {
		throw std::logic_error(
				"SpectralSequence::get_e_ab: Kernel is not set.");
	}
	if (cokers == cokernels_.end()) {
		throw std::logic_error(
				"SpectralSequence::get_e_ab: Cokernel is not set.");
	}
	if (kers->second.get_current() < a) {
		throw std::logic_error(
				"SpectralSequence::get_e_ab: Kernel is at wrong r.");
	}
