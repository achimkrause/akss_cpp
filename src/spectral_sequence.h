...
{
 public:
  GroupSequence(const std::size_t index_min, const AbelianGroup<P>& grp);
  const AbelianGroup<P>& get_group(const std::size_t index);
  const MatrixQ& get_matrix(const std::size_t index);
  void append(const std::size_t index, const AbelianGroup<P>& grp,
              const MatrixQ& map);
  void done();
 ...
 private:
  bool done_;
  std::map<std::size_t, std::tuple<AbelianGroup<P>, MatrixQ>> entries_;
  std::size_t current_;
...
class SpectralSequence
{
 public:
  ...
  const AbelianGroup<P> get_e_ab(TrigradedIndex pqs, std::size_t a, std::size_t b);

 private:
  std::map<TrigradedIndex, GroupSequence<P>> kernels_;
  std::map<TrigradedIndex, GroupSequence<P>> cokernels_;
